...
export async function retrofitExtension(targetDir: string): Promise<string> {
	...
	try {
		...
		// Very naive JSON parse (fails on comments), but good enough for a basic check,
		// or we can use regex. Since tsconfig often has comments, let's use regex.
	...
	// 4. Husky setup
	try {
		await execFileAsync("npm", ["install", "--save-dev", "husky@^9.0.0"], { cwd: absoluteTarget });
		log("Installed Husky v9");
		await execFileAsync("npx", ["husky", "init"], { cwd: absoluteTarget });

		const precommitPath = path.join(absoluteTarget, ".husky", "pre-commit");
		await fs.writeFile(precommitPath, "npx biome check --write .\ngit add -A\nnpx tsc --noEmit\n");
